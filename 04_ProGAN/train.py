...
def train_fn(
        critic, gen, loader, dataset, step, alpha, opt_critic, epoch,
        opt_gen, tensorboard_step, writer, scaler_gen, scaler_critic,):

    loop = tqdm(loader, leave=True)

    if epoch == 0: ###
        opt_critic.param_groups[0]['lr'] = 1e-3
        opt_gen.param_groups[0]['lr'] = 1e-3
        print(f'learning rate set to {opt_gen.param_groups[0]["lr"]}')
    elif epoch == 15: ###
        opt_critic.param_groups[0]['lr'] = 1e-5
        opt_gen.param_groups[0]['lr'] = 1e-5
        print(f'learning rate set to {opt_gen.param_groups[0]["lr"]}')

    alpha = epoch/config.PROGRESSIVE_EPOCHS[step] ###

    max_batch_idx = 1000/loader.batch_size
